...
)
{
    ...
    for (int n = 0; n < graph.getAdjNodes()[startIdx].size(); n++)
    {
        ...
        if (!tVisited[nodeIdx])
        {
            ...
            float costNode =
                costRoot +
                costMatrix[startIdx].at(nodeIdx) +
                r;
            ...
            if (costNode <= lowerBound)
            {
                ...
                if (tLevelIdx == graph.getNumNodes() - 1)
                {
                    ...
                    for (int i = 0; i < tPathSequence.size(); i++)
                    {
                        ...
                        {
                            finalCost +=
                                graph.getNodeDistance()
                                [tPathSequence.at(i - 1)]
                                .at(tPathSequence.at(i));
                        }
                    }
...
void BB(int startIdx, Graph graph)
{
    ...
    vector<float>* costMatrix =
        new vector<float>[graph.getNumNodes()];
    ...
    for (int i = 0; i < graph.getNumNodes(); i++)
    {
        ...
        for (int j = 0; j < graph.getNumNodes(); j++)
        {
            costMatrix[i].push_back(
                graph.getCostMatrix()[i].at(j)
            );
        }
    }
...
int main(int argc, char** argv)
{
    ...
    graph.addEdge(0, 2, 18.0f);
    graph.addEdge(0, 3, 12.0f);
    graph.addEdge(0, 4, 15.0f);
    graph.addEdge(1, 2, 15.0f);
    graph.addEdge(1, 3, 20.0f);
    graph.addEdge(1, 4, 11.0f);
    graph.addEdge(2, 3, 14.0f);
    graph.addEdge(2, 4, 16.0f);
    graph.addEdge(4, 3, 13.0f);
    ...
    // posisi jendela di layar komputer dalam piksel
    glutInitWindowPosition(100, 100);

    // judul jendela
    glutCreateWindow(
        "LUTFAN ALAUDIN NAJA / 2400018032 - PRAKTIKUM SA"
    );
